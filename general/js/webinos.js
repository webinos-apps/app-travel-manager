...
(function() {
  ...
  var newJSONRPCObj = function(sessionId, id) {
    return{jsonrpc:"2.0", id:id || getNextID(sessionId)}
  };
  var getNextID = function(sessionId) {
    if(idCount == Number.MAX_VALUE) {
      idCount = 0
    }
    idCount++;
  ...
  ServiceDiscovery.prototype.findServices = function(serviceType, callback, options, filter) {
    ...
    function success(params) {
      ...
      if(typeof RemoteControl !== "undefined") {
        typeMap["http://webinos.org/api/remotecontrol"] = RemoteControl
      }
...
(function(exports) {
  ...
  exports.relative = function(from, to) {
    ...
    var fromParts = trim(exports.normalize(from).substr(1).split("/")), toParts = trim(exports.normalize(to).substr(1).split("/"));
...
(function(exports) {
  ...
  exports.Buffer.prototype.slice = function(start, end, contentType) {
    ...
    var newView = new Uint8Array(newBuffer), oldView = new Uint8Array(this._buffer);
...
(function() {
  ...
  EventsModule.prototype.addWebinosEventListener = function(listener, type, source, destination) {
    ...
    req.type = type;
    req.source = source;
    if(typeof req.source === "undefined" || req.source == null) {
      req.source = this.myAppID
    }
...
(function() {
  ...
  var _referenceMapping = new Array;
  var _vehicleDataIds = ["climate-all", "climate-driver", "climate-passenger", "climate-behind-driver", "climate-behind-passenger", "light-fog-front", "light-fog-rear", "light-signal-right", "light-signal-left", "light-signal-warn", "lights-hibeam", "light-parking", "light-head", "light-head", "destination-reached", "destination-changed", "destination-cancelled", "parksensors-front", "parksensors-rear", "gear", "tripcomputer", "wipers", "oillevel", "doors", "windows"];
...
(function() {
  RemoteControl = function(obj) {
    this.base = WebinosService;
    this.base(obj)
  };
  RemoteControl.prototype = new WebinosService;
  RemoteControl.prototype.bindService = function(bindCB, serviceId) {
    this.addEventListener = addEventListener;
    this.removeEventListener = removeEventListener;
    if(typeof bindCB.onBind === "function") {
      bindCB.onBind(this)
  ...
  var _referenceMapping = new Array;
  var _RemoteControlDataIds = ["command"];
  function addEventListener(RemoteControlDataId, eventHandler, capture) {
    if(_RemoteControlDataIds.indexOf(RemoteControlDataId) != -1) {
      var rpc = webinos.rpcHandler.createRPC(this, "addEventListener", RemoteControlDataId);
      rpc.fromObjectRef = Math.floor(Math.random() * 101);
      _referenceMapping.push([rpc.fromObjectRef, eventHandler]);
      console.log("# of references" + _referenceMapping.length);
      var callback = new RPCWebinosService({api:rpc.fromObjectRef});
      callback.onEvent = function(RemoteControlEvent) {
        eventHandler(RemoteControlEvent)
      };
      webinos.rpcHandler.registerCallbackObject(callback);
      webinos.rpcHandler.executeRPC(rpc)
    }else {
      console.log(RemoteControlDataId + " not found")
    }
  }
  function removeEventListener(RemoteControlDataId, eventHandler, capture) {
    var refToBeDeleted = null;
    for(var i = 0;i < _referenceMapping.length;i++) {
      console.log("Reference" + i + ": " + _referenceMapping[i][0]);
      console.log("Handler" + i + ": " + _referenceMapping[i][1]);
      if(_referenceMapping[i][1] == eventHandler) {
        var arguments = new Array;
        arguments[0] = _referenceMapping[i][0];
        arguments[1] = RemoteControlDataId;
        console.log("ListenerObject to be removed ref#" + refToBeDeleted);
        var rpc = webinos.rpcHandler.createRPC(this, "removeEventListener", arguments);
        webinos.rpcHandler.executeRPC(rpc, function(result) {
          callOnSuccess(result)
